!function(kipp, window, document){
  ...
  kipp.toQueryString = function(object, base){
    // adapted from MooTools
    var queryString = []

    kipp.each(object, function(value, key){
      if (base)
        key = base + '[' + key + ']'

      var result

      if (kipp.isArray(value)){
        var qs = {}
        kipp.each(value, function(val, i){
          qs[i] = val
        })
        result = kipp.toQueryString(qs, key)
      }
      else if (typeof value == 'object')
        result = kipp.toQueryString(value, key)
      else
  ...
  kipp.request = function (options, callback){
    // adapted from MooTools
    if (!(this instanceof kipp.request))
      return new kipp.request(options, callback)

    var self = this
    self.options = kipp.extend({}, self.options, options)
  ...
  kipp.request.prototype = {
    ...
    send: function(){
      ...
      if (typeof data != 'string')
        data = kipp.toQueryString(data)

      if (options.emulation && kipp.indexOf(method, ['get', 'post']) < 0){
        var _method = '_method=' + method
        data = (data) ? _method + '&' + data : _method
        method = 'post'
      }

      if (options.urlEncoded && kipp.indexOf(method, ['post', 'put']) > -1){
      ...
      xhr.onreadystatechange = kipp.bind(self.onStateChange, self)
