!function (kipp, window, document){
  ...
  kipp.listener = function (params, handler){
    if (params.delegate){
      params.capture = true
      _handler = handler
      handler = function (event){
        ...
        var target = event.target || event.srcElement
          , nodes = typeof params.delegate == 'string'
            ? kipp.wrap(params.delegate, params.node)
            : params.delegate(params.node)

        while (target && kipp.indexOf(target, nodes) == -1 )
    ...
    if (params.context)
      handler = kipp.bind(handler, params.context)
  ...
  function init(e) {
    ...
    if (!ready && (ready = true))
      kipp.each(readyHandlers, function (handler){
        handler.apply(document)
      })
  }
